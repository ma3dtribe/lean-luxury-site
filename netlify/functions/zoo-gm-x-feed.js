...
function scoreZooRelevance(text = "", zooPlayers = []) {
  ...
  if (zooPlayers.length === 0) return 0;
...
function buildPostIntelligence(post) {
  ...
  const fantasyRelevance = scoreFantasyRelevance(combinedText, zooPlayers);
  const zooRelevance = scoreZooRelevance(combinedText, zooPlayers);
  ...
  return {
    ...
    intelligence: {
      ...
      fantasyRelevance,
      zooRelevance,
      alertLevel: getAlertLevel(fantasyRelevance, zooRelevance),
  ...
  try {
    ...
    const summary = {
      ...
      fantasyRelevant: posts.filter(
        (post) => post.intelligence.fantasyRelevance >= 60
      ).length,
      zooRelevant: posts.filter(
        (post) => post.intelligence.zooRelevance >= 60
      ).length,
      urgent: posts.filter(
        (post) => post.intelligence.alertLevel === "URGENT"
      ).length,
      important: posts.filter(
        (post) => post.intelligence.alertLevel === "IMPORTANT"
      ).length
    };
